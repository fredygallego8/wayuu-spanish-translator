...
export class PdfProcessingService implements OnModuleInit {
  ...
  extractDictionaryEntries(forceRefresh: boolean = false): Array<{ guc: string; spa: string; source: string; confidence: number }> {
    ...
    const startTime = Date.now();
    this.logger.log('🔄 Starting comprehensive dictionary extraction from all linguistic PDFs...');
    
    const dictionaryEntries = [];
    let processedPDFs = 0;
    const maxProcessingTime = 60000; // 🔧 AUMENTADO: 60 segundos máximo (antes 30)
    
    for (const pdfContent of this.pdfCache.values()) {
      // 🔧 EXPANDIDO: Procesar TODOS los PDFs que contengan contenido wayuu relevante
      // Priorizar diccionarios pero incluir manuales, cartillas y textos lingüísticos
      const fileName = pdfContent.fileName.toLowerCase();
      const isRelevantPDF = fileName.includes('dict') || 
                           fileName.includes('manual') || 
                           fileName.includes('cartilla') || 
                           fileName.includes('wayuu') ||
                           fileName.includes('gramatica') ||
                           fileName.includes('linguistica');
      
      if (!isRelevantPDF) {
        this.logger.debug(`⏭️ Skipping non-linguistic PDF: ${pdfContent.fileName}`);
        continue;
      }
      ...
      this.logger.log(`📖 Processing linguistic PDF: ${pdfContent.fileName}`);
      ...
      // 🔧 LÍMITE AUMENTADO: Máximo 4 PDFs para procesar todos los disponibles
      if (processedPDFs >= 4) {
    ...
    this.logger.log(`✅ Comprehensive extraction completed: ${uniqueEntries.length} entries in ${processingTime}ms from ${processedPDFs} PDFs (cached for ${this.EXTRACTED_ENTRIES_CACHE_TTL / 60000} minutes)`);
  ...
  private extractWayuuDictionaryFormat(text: string, fileName: string): Array<{ guc: string; spa: string; source: string; confidence: number }> {
    ...
    const maxLines = 10000; // 🔧 AUMENTADO: Límite de líneas para procesar más contenido
    ...
    for (let i = 0; i < linesToProcess; i++) {
      ...
      // 🔧 PATRÓN 1: Formato diccionario específico (funciona para WayuuDict_45801.pdf)
      ...
      // 🆕 PATRÓN 2: Formato conversacional (para cartillas educativas)
      // Ejemplo: W: [antüsü pia] "Has llegado"
      const conversationalMatch = currentLine.match(/([WM]):\s*\[([^\]]+)\]\s*[""]([^""]+)[""]?/);
      if (conversationalMatch) {
        const wayuu = this.cleanPhraseAdvanced(conversationalMatch[2]);
        const spanish = this.cleanPhraseAdvanced(conversationalMatch[3]);
        
        if (this.isValidDictionaryEntryAdvanced(wayuu, spanish)) {
          entries.push({
            guc: wayuu,
            spa: spanish,
            source: `PDF:${fileName}:ConversationalFormat`,
            confidence: this.calculateAdvancedConfidence(wayuu, spanish, fileName, 'conversational')
          });
        }
      }
      
      // 🆕 PATRÓN 3: Formato directo con comillas
      // Ejemplo: tanülia "mi nombre"
      const directQuotesMatch = currentLine.match(/([a-züÜ'ꞌ]+(?:\s+[a-züÜ'ꞌ]+)*)\s+[""]([^""]+)[""]?/);
      if (directQuotesMatch) {
        const wayuu = this.cleanPhraseAdvanced(directQuotesMatch[1]);
        const spanish = this.cleanPhraseAdvanced(directQuotesMatch[2]);
        
        if (this.isValidDictionaryEntryAdvanced(wayuu, spanish)) {
          entries.push({
            guc: wayuu,
            spa: spanish,
            source: `PDF:${fileName}:DirectQuotes`,
            confidence: this.calculateAdvancedConfidence(wayuu, spanish, fileName, 'direct_quotes')
          });
        }
      }
      
      // 🆕 PATRÓN 4: Formato de traducción con separador /
      // Ejemplo: ja.mü.che' /jamüche'/ "tuna"
      const slashSeparatorMatch = currentLine.match(/([a-züÜ'ꞌ.]+)\s+\/([^\/]+)\/\s+[""]([^""]+)[""]?/);
      if (slashSeparatorMatch) {
        const wayuu = this.cleanPhraseAdvanced(slashSeparatorMatch[1].replace(/\./g, ''));
        const spanish = this.cleanPhraseAdvanced(slashSeparatorMatch[3]);
        
        if (this.isValidDictionaryEntryAdvanced(wayuu, spanish)) {
          entries.push({
            guc: wayuu,
            spa: spanish,
            source: `PDF:${fileName}:SlashSeparator`,
            confidence: this.calculateAdvancedConfidence(wayuu, spanish, fileName, 'slash_separator')
          });
        }
      }
  ...
  private calculateAdvancedConfidence(wayuu: string, spanish: string, fileName: string, patternType: string): number {
    ...
    switch (patternType) {
      case 'dictionary_format': confidence += 0.25; break; // Formato específico diccionario wayuu
      case 'conversational': confidence += 0.22; break; // Formato conversacional de cartillas
      case 'direct_quotes': confidence += 0.20; break; // Formato directo con comillas
      case 'slash_separator': confidence += 0.18; break; // Formato con separador /

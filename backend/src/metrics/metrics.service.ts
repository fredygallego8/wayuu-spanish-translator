...
export class MetricsService implements OnModuleInit {
  ...
  // Métricas específicas de Gemini Dictionary
  public readonly geminiGeneratedEntries = new Gauge({
    name: 'wayuu_gemini_generated_entries_total',
    help: 'Total number of entries generated by Gemini',
  });

  public readonly geminiIntegratedEntries = new Gauge({
    name: 'wayuu_gemini_integrated_entries_total',
    help: 'Total number of Gemini entries integrated into main dictionary',
  });

  public readonly geminiSessions = new Gauge({
    name: 'wayuu_gemini_sessions_total',
    help: 'Total number of Gemini expansion sessions',
  });

  public readonly geminiAverageConfidence = new Gauge({
    name: 'wayuu_gemini_average_confidence',
    help: 'Average confidence score of Gemini generated entries',
  });

  public readonly geminiPendingReview = new Gauge({
    name: 'wayuu_gemini_pending_review_total',
    help: 'Number of Gemini entries pending manual review',
  });

  public readonly geminiLastExpansion = new Gauge({
    name: 'wayuu_gemini_last_expansion_timestamp',
    help: 'Timestamp of last Gemini dictionary expansion',
  });

  public readonly geminiApiCalls = new Counter({
    name: 'wayuu_gemini_api_calls_total',
    help: 'Total number of calls to Gemini API',
    labelNames: ['status', 'domain'],
  });

  public readonly geminiResponseTime = new Histogram({
    name: 'wayuu_gemini_response_time_seconds',
    help: 'Response time of Gemini API calls in seconds',
    labelNames: ['domain'],
  ...
  onModuleInit() {
    ...
    // Registrar métricas de Gemini Dictionary
    register.registerMetric(this.geminiGeneratedEntries);
    register.registerMetric(this.geminiIntegratedEntries);
    register.registerMetric(this.geminiSessions);
    register.registerMetric(this.geminiAverageConfidence);
    register.registerMetric(this.geminiPendingReview);
    register.registerMetric(this.geminiLastExpansion);
    register.registerMetric(this.geminiApiCalls);
    register.registerMetric(this.geminiResponseTime);
  }
  ...
  updateGrowthMetric(metricName: string, value: number) {
    switch (metricName) {
      case 'wayuu_total_words_wayuu':
        this.totalWayuuWords.set(value);
        break;
      case 'wayuu_total_words_spanish':
        this.totalSpanishWords.set(value);
        break;
      case 'wayuu_total_audio_minutes':
        this.totalAudioMinutes.set(value);
        break;
      case 'wayuu_total_phrases':
        this.totalPhrases.set(value);
        break;
      case 'wayuu_total_transcribed':
        this.totalTranscribed.set(value);
        break;
      case 'wayuu_total_dictionary_entries':
        this.totalDictionaryEntries.set(value);
        break;
      case 'wayuu_total_audio_files':
        this.totalAudioFiles.set(value);
        break;
      case 'wayuu_growth_last_update_timestamp':
        this.growthLastUpdateTimestamp.set(value);
        break;
    }
  }

  // Métodos para métricas de Gemini Dictionary
  updateGeminiGeneratedEntries(total: number) {
    this.geminiGeneratedEntries.set(total);
  }

  updateGeminiIntegratedEntries(total: number) {
    this.geminiIntegratedEntries.set(total);
  }

  updateGeminiSessions(total: number) {
    this.geminiSessions.set(total);
  }

  updateGeminiAverageConfidence(confidence: number) {
    this.geminiAverageConfidence.set(confidence);
  }

  updateGeminiPendingReview(total: number) {
    this.geminiPendingReview.set(total);
  }

  updateGeminiLastExpansion(timestamp: number) {
    this.geminiLastExpansion.set(timestamp);
  }

  incrementGeminiApiCall(status: string, domain: string) {
    this.geminiApiCalls.inc({ status, domain });
  }

  recordGeminiResponseTime(domain: string, duration: number) {
    this.geminiResponseTime.observe({ domain }, duration);
  }

  // Método auxiliar para actualizar todas las métricas de Gemini de una vez
  updateGeminiStats(stats: {
    ...
    this.updateGeminiGeneratedEntries(stats.totalGenerated);
    this.updateGeminiIntegratedEntries(stats.totalIntegrated);
    this.updateGeminiSessions(stats.totalSessions);
    this.updateGeminiAverageConfidence(stats.averageConfidence);
    this.updateGeminiPendingReview(stats.pendingReview);
    
    if (stats.lastExpansion) {
      this.updateGeminiLastExpansion(new Date(stats.lastExpansion).getTime());
    }
  }
}
